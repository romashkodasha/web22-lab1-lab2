...
from bmstu_lab.models import Groups
from bmstu_lab.models import Students
from bmstu_lab.models import Subscriptions
from bmstu_lab.models import Timetable
from bmstu_lab.models import Trainers
...
def GetGroup(request,id):
    return render(request,'group.html',{'data':{
        'group':Groups.objects.filter(id=id)[0],
        'trainer':Groups.objects.get(id=id).id_trainer,
        'timetable':Timetable.objects.filter(id_group=id)
    }})
